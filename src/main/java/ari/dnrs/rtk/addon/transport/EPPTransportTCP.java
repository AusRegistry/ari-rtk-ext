...
/**
 * Provides methods necessary to establish connection and to communicate with an EPP Server using plain socket (no
 * encryption).
 */
public class EPPTransportTCP extends EPPTransportBase {
    ...
    /**
     * Constructor with established socket and timeout value If the timeout
     * value is zero, the default timeout value from EPPTransportBase is used.
     * 
     * @param socket
     *            The Socket to the server
     * @param timeout
    ...
    /**
     * Constructor with Hostname, Host port and timeout value If the timeout
     * value is zero, the default timeout value from EPPTransportBase is used.
     * 
     * @param host_name
     *            The server Hostname
    ...
    /**
     * Connects to the Server using previously set Hostname and port. If the socket was provided externally, the
     * connection operation is skipped, but the input and output buffers are still extracted. The method also sets the
     * SO timeout of the socket regardless of its origins.
     */
    @Override
     ...
     *
     * @throws org.openrtk.idl.epprtk.epp_Exception
     *             if there was a socket error in writing to the EPP Server. The epp_Exception will contain a result
     *             with the code epp_Session.RTK_COMMUNICATIONS_FAILURE
     * @see org.openrtk.idl.epprtk.epp_Session#RTK_COMMUNICATIONS_FAILURE
     */
    @Override
    ...
    public void disconnect() throws IOException {
        ...
        try {
            if (socket_to_server_ != null) {
                socket_to_server_.close();
            }
            socket_to_server_ = null;

            if (reader_from_server_ != null) {
                reader_from_server_.close();
            }
            reader_from_server_ = null;

            if (writer_to_server_ != null) {
                writer_to_server_.close();
            }
     ...
     * @throws Exception
     *             if there was a Socket problem or less than 4 bytes read
     */
